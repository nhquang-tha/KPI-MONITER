...
<div class="card">
    ...
    <div class="card-body">
                ...
                <div class="col-md-12">
                     <form method="GET" action="/traffic-down" class="row g-3 align-items-center bg-light p-3 rounded-3 border">
                        ...
                        <div class="col-auto">
                         ...
                         <div class="col-auto">
                            <button type="submit" name="action" value="execute" class="btn btn-primary shadow-sm"><i class="fa-solid fa-play me-2"></i>Thực hiện</button>
                        </div>
                        ...
                        <div class="card-body p-0 table-responsive">
                            <table class="table table-striped mb-0 small">
                                ...
                                <tbody>
                                    ...
                                    {% else %}
                                    <tr><td colspan="4" class="text-center text-muted py-3">Không có dữ liệu (Nhấn 'Thực hiện' để lọc)</td></tr>
                                    {% endfor %}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="card-body p-0 table-responsive">
                            <table class="table table-striped mb-0 small">
                                ...
                                <tbody>
                                    ...
                                    {% else %}
                                    <tr><td colspan="5" class="text-center text-muted py-3">Không có dữ liệu (Nhấn 'Thực hiện' để lọc)</td></tr>
                                    {% endfor %}
                                </tbody>
                            </table>
                        </div>
        ...
        {% elif active_page == 'conges_3g' %}
            <div class="row mb-4">
                 <div class="col-md-12">
                     <form method="GET" action="/conges-3g" class="d-flex align-items-center">
                         <div class="alert alert-info border-0 shadow-sm bg-soft-primary text-primary mb-0 flex-grow-1">
                            <i class="fa-solid fa-circle-info me-2"></i><strong>Điều kiện:</strong> (CS_CONG > 2% & CS_ATT > 100) OR (PS_CONG > 2% & PS_ATT > 500) (3 ngày liên tiếp)
                         </div>
                         <button type="submit" name="action" value="execute" class="btn btn-primary shadow-sm ms-3"><i class="fa-solid fa-play me-2"></i>Thực hiện</button>
                     </form>
                 </div>
            </div>
            
            {% if dates %}
                <div class="mb-3 text-muted small"><i class="fa-solid fa-calendar me-2"></i>Xét duyệt: {% for d in dates %}<span class="badge bg-light text-dark border ms-1">{{ d }}</span>{% endfor %}</div>
            {% endif %}
            ...
            <div class="table-responsive bg-white rounded shadow-sm border">
                <table class="table table-hover mb-0" style="font-size: 0.9rem;">
                    ...
                    <tbody>
                        ...
                        {% else %}
                        <tr><td colspan="6" class="text-center py-5 text-muted opacity-50">
                            {% if dates %}
                                <i class="fa-solid fa-face-smile fa-2x mb-2 d-block"></i>Tuyệt vời! Không có cell nghẽn.
                            {% else %}
                                Nhấn nút "Thực hiện" để bắt đầu phân tích.
                            {% endif %}
                        </td></tr>
                        {% endfor %}
                    </tbody>
...
def conges_3g():
    ...
    # Chỉ thực hiện tính toán khi có action 'execute'
    ...
    return render_page(CONTENT_TEMPLATE, title="Congestion 3G", active_page='conges_3g', conges_data=conges_data, dates=target_dates_str)
...
def traffic_down():
    ...
    action = request.args.get('action') # Check action
    
    zero_traffic_cells = []
    degraded_cells = []
    analysis_date_str = "N/A"

    # Chỉ thực hiện logic nặng khi có action='execute'
    if action == 'execute':
        ...
        if sorted_dates:
            ...
            analysis_date_str = latest_date.strftime('%d/%m/%Y')
    ...
    return render_page(CONTENT_TEMPLATE, title="Traffic Down Analysis", active_page='traffic_down', 
                       ...
                       tech=tech, analysis_date=analysis_date_str)
