...
# ==============================================================================
# 4. TEMPLATES (BẢN ĐẦY ĐỦ KHÔNG BỊ TRUNCATE)
# ==============================================================================
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="/gis"><i class="fa-solid fa-map-location-dot"></i> Bản đồ GIS</a></li>
            <li><a href="/kpi"><i class="fa-solid fa-chart-line"></i> KPI Analytics</a></li>
            <li><a href="/qoe-qos"><i class="fa-solid fa-star-half-stroke"></i> QoE QoS Analytics</a></li>
            <li><a href="/optimize"><i class="fa-solid fa-wand-magic-sparkles"></i> Tối ưu QoE/QoS</a></li>
            <li><a href="/rf"><i class="fa-solid fa-tower-broadcast"></i> RF Database</a></li>
            <li><a href="/poi"><i class="fa-solid fa-map-pin"></i> POI Report</a></li>
            <li><a href="/worst-cell"><i class="fa-solid fa-triangle-exclamation"></i> Worst Cells</a></li>
            <li><a href="/conges-3g"><i class="fa-solid fa-users-slash"></i> Congestion 3G</a></li>
            <li><a href="/traffic-down"><i class="fa-solid fa-arrow-trend-down"></i> Traffic Down</a></li>
            <li>
                ...
                <ul class="collapse list-unstyled" id="toolsMenu">
                    <li><a href="/azimuth" style="margin-left: 20px; font-size: 0.95rem;"><i class="fa-solid fa-compass"></i> Azimuth</a></li>
                    <li><a href="/script" style="margin-left: 20px; font-size: 0.95rem;"><i class="fa-solid fa-code"></i> Script</a></li>
                </ul>
            </li>
            {% if current_user.role == 'admin' %}
            <li><a href="/import"><i class="fa-solid fa-cloud-arrow-up"></i> Data Import</a></li>
            ...
            {% with messages = get_flashed_messages(with_categories=true) %}
                {% if messages %}
                    {% for category, message in messages %}
                        <div class="alert alert-{{ category }} alert-dismissible fade show shadow-sm border-0 mb-4" role="alert">{{ message }}<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
                    {% endfor %}
                {% endif %}
            {% endwith %}
    ...
    <div class="modal fade" id="chartDetailModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="background: rgba(255,255,255,0.95); backdrop-filter: blur(15px);">
                <div class="modal-header border-0 pb-0"><h5 class="modal-title text-primary fw-bold" id="modalTitle"></h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
    ...
    <script>
        ...
        function toggleCheckboxes(source) { let checkboxes = document.getElementsByName('tables'); for(var i=0; i<checkboxes.length; i++) checkboxes[i].checked = source.checked; }
        function addRow(tech) {
            ...
            var defaultSRN = tech == '3g900' ? 70 + idx - 1 : (tech == '3g2100' ? 80 + idx - 1 : 60 + idx - 1);
            var defaultSlot = tech == '3g900' ? 2 : 3;
...
<head>
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>body { background: linear-gradient(135deg, #f0f2f5 0%, #d9e2ec 100%); height: 100vh; display: flex; align-items: center; justify-content: center; font-family: 'Segoe UI', sans-serif; } .login-card { width: 100%; max-width: 400px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(20px); padding: 40px; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.08); border: 1px solid rgba(255,255,255,0.5); } .btn-primary { background-color: #0078d4; border: none; padding: 10px; font-weight: 600; border-radius: 8px; transition: all 0.3s; width: 100%; } .btn-primary:hover { background-color: #0063b1; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0, 120, 212, 0.3); } .form-control { border-radius: 8px; padding: 12px; margin-bottom: 1rem; border: 1px solid #e0e0e0; } .form-control:focus { box-shadow: 0 0 0 3px rgba(0, 120, 212, 0.15); border-color: #0078d4; }</style>
</head>
<body>
    <div class="login-card">
        ...
        <form method="POST"><label class="form-label fw-bold text-secondary small">USERNAME</label><input type="text" name="username" class="form-control" placeholder="Enter username" required><label class="form-label fw-bold text-secondary small">PASSWORD</label><input type="password" name="password" class="form-control" placeholder="Enter password" required><button type="submit" class="btn btn-primary">Sign In</button></form>
    </div>
</body>
</html>
"""
        ...
        {% elif active_page == 'optimize' %}
            ...
            <div class="table-responsive bg-white rounded shadow-sm border" style="max-height: 65vh;">
                 <table class="table table-hover table-bordered mb-0 align-middle" style="font-size: 0.85rem;">
                     <thead class="table-light text-center position-sticky top-0" style="z-index: 10;"><tr><th rowspan="2" class="align-middle">Cell Name</th><th colspan="2">1. Báo cáo Tuần (Macro)</th><th colspan="4">2. KPI Ngày Gần Nhất (Micro)</th><th rowspan="2" class="align-middle">3. Chẩn đoán (Bệnh)</th><th rowspan="2" class="align-middle">4. Giải pháp (Action)</th><th rowspan="2" class="align-middle">5. Giám sát</th></tr><tr><th>Điểm QoE</th><th>Điểm QoS</th><th>PRB (%)</th><th>Thput (Mbps)</th><th>CQI (%)</th><th>Drop (%)</th></tr></thead>
                     <tbody>
                         ...
                         <tr>
                             ...
                             <td><ul class="mb-0 ps-3 text-danger fw-bold" style="min-width: 150px;">{% for issue in row.issues %}<li>{{ issue }}</li>{% endfor %}</ul></td>
                             <td><ul class="mb-0 ps-3 text-success" style="min-width: 180px;">{% for action in row.actions %}<li>{{ action }}</li>{% endfor %}</ul></td>
                             <td class="text-center p-2" style="min-width: 100px;"><a href="/kpi?tech=4g&cell_name={{ row.cell_name }}" class="btn btn-sm btn-outline-primary mb-1 w-100 py-0 shadow-sm" style="font-size: 0.75rem;"><i class="fa-solid fa-chart-line me-1"></i>Xem KPI</a><a href="/qoe-qos?cell_name={{ row.cell_name }}" class="btn btn-sm btn-outline-warning w-100 py-0 text-dark shadow-sm" style="font-size: 0.75rem;"><i class="fa-solid fa-star-half-stroke me-1"></i>Xem QoE</a></td>
                         </tr>
                         {% else %}
        ...
        {% elif active_page == 'gis' %}
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    var hasGisData = gisData.length > 0; var hasItsData = isShowIts && itsData.length > 0;
                    if(!document.getElementById('gisMap')) return;
                    var mapCenter = [19.807, 105.776]; var mapZoom = 9;
                    var map = L.map('gisMap', {center: mapCenter, zoom: mapZoom, zoomControl: true, fullscreenControl: true, fullscreenControlOptions: { position: 'topleft' }});
                    var googleStreets = L.tileLayer('https://{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', {maxZoom: 22, subdomains: ['mt0', 'mt1', 'mt2', 'mt3'], detectRetina: true, attribution: '© Google Maps'}).addTo(map);
                    var googleHybrid = L.tileLayer('https://{s}.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', {maxZoom: 22, subdomains: ['mt0', 'mt1', 'mt2', 'mt3'], detectRetina: true, attribution: '© Google Maps'});
                    ...
                    if (actionType === 'search' && (searchSite || searchCell) && gisData.length > 0) {
                        ...
                        if (targetCell.lat && targetCell.lon) map.setView([targetCell.lat, targetCell.lon], 15); else map.setView(mapCenter, mapZoom);
                    } else if (bounds.length > 0) { map.fitBounds(bounds, {padding: [30, 30], maxZoom: 16}); } else { map.setView(mapCenter, mapZoom); }
                    ...
                    function getSectorMidPoint(lat, lon, azimuth, distanceMeters) { var latFactor = 111320; var lonFactor = 111320 * Math.cos(lat * Math.PI / 180); var radMap = (azimuth || 0) * Math.PI / 180; return [lat + (distanceMeters * Math.cos(radMap)) / latFactor, lon + (distanceMeters * Math.sin(radMap)) / lonFactor]; }
                    ...
                    function buildLookupAndDrawSectors() {
                        ...
                        var targetCellCoord = null; var targetPolygon = null;

                        gisData.forEach(function(cell) {
                            if(!cell.lat || !cell.lon) return;
                            var isMatch = false;
                            if (actionType === 'search' && (searchSite || searchCell)) {
                                if ((searchSite && (cell.site_code || "").toLowerCase().includes(searchSite.toLowerCase())) || (searchCell && (cell.cell_name || "").toLowerCase().includes(searchCell.toLowerCase()))) { isMatch = true; if (!targetCellCoord) targetCellCoord = [cell.lat, cell.lon]; }
                            }
                            var key = "";
                            ...
                            if (key) cellLookup[key] = getSectorMidPoint(cell.lat, cell.lon, cell.azi, sectorRadius * 0.65);
                        ...
                        if (actionType === 'search' && targetCellCoord) { map.setView(targetCellCoord, 16); if (targetPolygon) setTimeout(() => targetPolygon.openPopup(), 600); }
                        drawITSData();
                    }

                    function getSignalColor(tech, level) {
                        var t = (tech || '').toUpperCase();
                        if (t.includes('4G') || t.includes('LTE')) { if (level >= -75) return '#0000FF'; if (level >= -85) return '#00FF00'; if (level >= -95) return '#FFFF00'; if (level >= -105) return '#FFA500'; if (level >= -115) return '#FF0000'; return '#000000'; }
                        else { if (level >= -65) return '#0000FF'; if (level >= -75) return '#00FF00'; if (level >= -85) return '#FFFF00'; if (level >= -95) return '#FFA500'; if (level >= -105) return '#FF0000'; return '#000000'; }
                    }
                    ...
                    function drawITSData() {
                        ...
                        itsData.forEach(function(pt) {
                            var ptColor = getSignalColor(pt.tech, pt.level); var ptCoord = [pt.lat, pt.lon];
                            L.circleMarker(ptCoord, {radius: pointSize, fillColor: ptColor, color: "#000", weight: 0.5, fillOpacity: 0.9}).bindPopup("<div class='small'><b>Tech:</b> " + pt.tech + "<br><b>Level:</b> <span class='fw-bold' style='color:"+ptColor+"'>" + pt.level + " dBm</span><br><b>Qual:</b> " + (pt.qual||'-') + "<br><b>Node/CellID:</b> " + (pt.node||'-') + " / " + pt.cellid + "</div>").addTo(itsLayerGroup);
                            if (showLines) {
                                var ptTech = (pt.tech || '').toLowerCase(); var key = "";
                                if (ptTech.includes('4g') || ptTech.includes('lte')) { if (pt.node && pt.cellid) key = pt.node + "_" + pt.cellid; }
                                else if (ptTech.includes('3g') || ptTech.includes('wcdma') || ptTech.includes('hspa') || ptTech.includes('umts')) { if (pt.cellid) key = pt.cellid; }
                                else if (ptTech.includes('5g') || ptTech.includes('nr')) { if (pt.node && pt.cellid) key = pt.node + "_" + pt.cellid; }
                                if (cellLookup[key]) L.polyline([ptCoord, cellLookup[key]], {color: ptColor, weight: 1.5, opacity: 0.6, dashArray: '3, 4'}).addTo(itsLayerGroup);
                            }
                        });
                    }
